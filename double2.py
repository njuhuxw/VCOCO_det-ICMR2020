...
# final version: no problem, and fix the x,y,w,h
        ...
        for i in test: #test is action
            ...
            if 'obj' in role_name and 'instr' in role_name:
                ...
                for ii in range(len(humans)):
                    ...
                    if iou_human > 0.5:
                        ...
                        for iii in range(len(oboxs_det)):
                            ...
                            sort_action=action_sort(action_score[iii])
                            if len(obox_det) != 0 and len(obox_ann) != 0:
                                iou_object = compute_iou(obox_ann, obox_det)

                            # if  iou_object > 0.5 and action_name in sort_action[:3]:
                            #     for mm in range(3):
                            #         if action_name == sort_action[mm] and len(action_name)>0 and len(iou_cls_obj)>0:
                            #             show_boxes(im_path, str(dimid) + action_name + str(mm + 1),
                            #                        [hbox_det, obox_det],
                            #                        [action_name, iou_cls_obj],
                            #                        ['red', '#00B0F0'])
                            if iou_human > 0.5 and iou_object > 0.5 and len(iou_cls_obj) > 0 and len(
                                        action_name) > 0:
                                    print(action_name)
                                    sum += 1
                                    print ("sum : " + str(sum))
                ...
                if len(verbb) > 0 and len(iou_cls_obj) > 0:
                    show_boxes(im_path, str(dimid) + action_name + sum, [hbox_det, obox_det],
                                           [vrbs, iou_cls_obj],
                                           ['red', '#00B0F0'])
